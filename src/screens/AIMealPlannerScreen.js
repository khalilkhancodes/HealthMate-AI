    ...
    <View>
      {lines.map((line, i) => {
        ...
        if (isHeading) {
          return (
            <View key={i} style={styles.richHeadingRow}>
              <View style={[styles.richHeadingAccent, { backgroundColor: colors.primary || colors.primary }]} />
              <Text style={[styles.richHeading, { color: colors.primary || colors.primary }]}>{trimmed}</Text>
            </View>
          );
        }
        ...
        if (trimmed.startsWith('- ') || trimmed.startsWith('• ')) {
          ...
          return (
            <View key={i} style={styles.richBulletRow}>
              <View style={[styles.richBulletDot, { backgroundColor: colors.tertiary }]} />
...
export default function AIMealPlannerScreen({ navigation }) {
  ...
  const handleEdit = (item) => {
    const history = aiMealPlannerHistory;
...
1. NO markdown (** or #).
2. Use ALL CAPS for section headings (if weekly then DAYS Names, If Food then FOOD ITEMS ). 
3. Add | before each heading.
4. Separate sections heading with a vertical blank line. Use discs for bullet points.

Example output:
| MONDAY:
• Grilled Chicken with brown rice and mixed vegetables - 120grams

| TUESDAY:
• Turkey with Avocado wrap - 800calories
...
Professional, practical and supportive.`;
  ...
  const callAPIWithStreaming = async (messages, onChunk) => {
    ...
    const fullText = data.choices?.[0]?.message?.content || 'Sorry, I could not generate a meal plan at this time.';
  ...
  const handleSend = async () => {
    ...
    if (isEdit) {
      useHealthStore.getState().updateMealPlannerMessage?.(messageId, userMessage);
    } else {
    ...
    try {
      ...
      if (isEdit && editPairRef.current[messageId]) {
        useHealthStore.getState().updateMealPlannerMessage?.(aiResponseId, aiMessage);
      } else {
        addMealPlannerMessage(aiMessage);
    ...
    return (
      <View style={[styles.messageWrapper, isUser ? styles.messageWrapperUser : styles.messageWrapperAI]}>
        ...
        <View style={styles.bubbleContainer}>
          ...
          >
            {isUser ? (
              <Text selectable={true} style={[styles.messageText, { color: COLORS.card }]}>
                {item.text}
              </Text>
            ) : (
              <View>
        ...
        >
          <View style={[
            ...
            <TextInput
              ...
              placeholder={editingMessageId ? 'Editing message...' : "E.g., High protein vegetarian dinner..."}
            ...
            <TouchableOpacity
              style={[styles.sendButton, { backgroundColor: inputText.trim() ? accentColor : accentColor + '20' }]}
              onPress={handleSend}
